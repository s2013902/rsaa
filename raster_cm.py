...
truth = label_all
predictt = predict_all

truth = np.reshape(truth,(-1,1))
predictt = np.reshape(predictt,(-1,1))

a1, b1, c1, d1 = train_test_split(truth, predictt, test_size=0.1, train_size = 0.9, random_state= 3)

#Undersampling
truth2 = np.reshape(b1,(-1,1))
predictt2 = np.reshape(d1, (-1,1))

rus = RandomUnderSampler(random_state=45)
X_rus, y_rus= rus.fit_resample(truth2,predictt2)

X_rus = X_rus.flatten()
y_rus = y_rus.flatten()

print(np.unique(X_rus, return_counts=True))
print(np.unique(y_rus, return_counts=True))


label_all = np.asarray(X_rus)
predict_all = np.asarray(y_rus)
